...
void* listening_ihm_tcp_thread(void* arg)
{
	while (1)
	{
		...
		{		
			...
			if (client_socket == INVALID_SOCKET) {
				...
				//sleep(10);
				perror("accept error"); // accept retourne immediatement 9 si cable coupé
					continue;
			//	break;
			}
	...
	/* 		setsockopt(client_socket,SOL_SOCKET,SO_LINGER,&one,sizeof(one));		// attend la fin de l'emission lors du close() */
			...
			//SO_SNDBUF et SO_RCVBUF sont respectivement des options permettant d'ajuster la taille des buffers alloués 
			//pour l'émission et la réception. La taille des buffers peut être augmentées pour des connexions avec un trafic important. Il y a des limites imposées par le système pour ces valeurs. 
